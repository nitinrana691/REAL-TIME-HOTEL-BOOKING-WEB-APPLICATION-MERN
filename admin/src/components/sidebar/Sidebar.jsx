...
const Sidebar = () => {
  ...
  const { dispatch: authDispatch } = useContext(AuthContext); // Get the dispatch function from AuthContext
  const navigate = useNavigate(); // Hook to programmatically navigate

  const handleLogout = () => {
    // Clear user authentication data (e.g., remove token from local storage)
    localStorage.removeItem('userToken'); // Adjust based on your implementation
    authDispatch({ type: 'LOGOUT' }); // Dispatch logout action (if using context)
